...
local function ShowActionsMenu(index, b)
    ...
    elseif bindType == "macro" then
        ...
        if Cell.isTWW then
            for idx=1,138 do
                local name, iconTexture, _ = GetMacroInfo(idx)
                if name then
                    tinsert(items, {
                        ["text"] = name,
                        ["icon"] = iconTexture,
...
CreateBindingListButton = function(modifier, bindKey, bindType, bindAction, i)
    ...
    elseif bindType == "macro" and Cell.isTWW then
        b.bindActionDisplay = GetMacroInfo(bindAction)
        b:ShowMacroIcon(bindAction)
    else
