...
ShouldShowPowerText = function(b)
    ...
    -- guid may be secret for NPC/follower units; `not secret` crashes.
    -- Use rawequal to safely check nil without boolean test on secrets.
    if rawequal(b.states.guid, nil) then
        return true
    end
