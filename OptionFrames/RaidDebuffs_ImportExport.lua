...
local importExportFrame = CreateFrame("Frame", "CellOptionsFrame_RaidDebuffsImportExport", Cell.frames.raidDebuffsTab, "BackdropTemplate")
...
if not Cell.frames.raidDebuffsTab.mask then
    Cell:CreateMask(Cell.frames.raidDebuffsTab)
...
-- instance name
local instance = importExportFrame:CreateFontString(nil, "OVERLAY", "CELL_FONT_CLASS")
instance:SetPoint("TOPLEFT", title, "BOTTOMLEFT", 0, -5)

-- boss name
local boss = importExportFrame:CreateFontString(nil, "OVERLAY", "CELL_FONT_CLASS")
boss:SetPoint("TOPLEFT", instance, "BOTTOMLEFT", 0, -5)

-- which bosses
local whichBossesBtn = Cell:CreateButton(importExportFrame, L["Current Boss"], "blue", {111, 18})
...
local textArea = Cell:CreateScrollEditBox(importExportFrame, function(eb, userChanged)
    if userChanged then
        if isImport then
            ...
            -- check
            local version, instanceId, bossId, data = string.match(text, "^!CELL:(%d+):(%d+):(.+)!(.+)$")
...
ShowData = function(instanceId, bossId)
    ...
    if data then
        ...
        local prefix = "!CELL:"..(tonumber(string.match(Cell.version, "%d+")) or 0)..":"..instanceId..":"..(bossId or "all").."!"
